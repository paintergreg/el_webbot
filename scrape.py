...
def downloadPDF(productID, href, abbreviations: list):
    ...
    postData = {}
    postData = {
        "__EVENTTARGET": "ctl00$MainContent$productRepeater$ctl00$MakePdfProduct"
    }
...
if __name__ == "__main__":
    ...
    if url is None:
        ...
        abbrevations = read_csv(
            "/home/greg/Dropbox/AAA/el-replacement-names.csv"
        )
        print(f"{url}")
        ...
        for link in mainPage.select("span.sizeSpan a"):
            ...
            if not colorChangeLink:
                ...
                pdf, mod_name = downloadPDF(
                    productID, designPageLink, abbrevations
                )
                # PDF is downloaded.  Save in a folder named with the productID
                # and productName.  Make the file name the productID,
                # productName and '.pdf' extension
                # The productName is called mod_name, it is ran
                # through an algorithm to abbreviate long names
                savePDF(base_dir, pdf, productID, mod_name)
